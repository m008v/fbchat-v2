...
Thư viện Python async-first để tự động hóa Facebook Messenger bằng cookie phiên đăng nhập. HTTP async dùng `httpx.AsyncClient`, adapter legacy chỉ nằm sau API async, MQTT/WebSocket xử lý tin nhắn thường và bridge Go riêng xử lý E2EE.
...
- API hiện hành ưu tiên `async`/`await` với tên public không hậu tố như `dataGetHome`, `func`, `send` và `connect_mqtt`.
...
`connect_mqtt()` đưa vòng lặp blocking của `paho-mqtt` sang worker thread dành riêng. Đây là adapter đúng cho một thư viện MQTT đồng bộ; các workflow HTTP async dùng `httpx.AsyncClient` và chỉ giữ adapter legacy ở boundary nội bộ khi Facebook bắt buộc.
...
Cookie là luồng nên dùng. Nếu buộc phải dùng login credentials, module sử dụng mặc định FB4A legacy và cho phép override token qua môi trường khi cần:
...
TOTP secret chỉ được xử lý trên máy bằng `pyotp`, không gửi đến `2fa.live` hay endpoint bên thứ ba. Credential login dùng form/header FB4A legacy giống bản đầu; `FBCHAT_APP_ACCESS_TOKEN` và `FBCHAT_API_KEY` chỉ là override tùy chọn.
