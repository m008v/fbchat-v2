...
``{"success": 1, ...}``, ``{"uncertain": 1, ...}`` hoặc ``{"error": 1, ...}``.
...
class api:
    ...
    def connect(
        ...
        try:
            ...
            bridge.call_blocking("newClient", cfg)
            info = bridge.call_blocking("connect", timeout=timeout)
            if enable_e2ee:
                bridge.call_blocking("connectE2EE", timeout=timeout)
        except BaseException:
            # Standalone mode owns this process. A failed handshake leaves no
            # usable state to reuse, so fail closed and reap it immediately.
    ...
    def send(
        ...
        if isinstance(delivery_status, str) and delivery_status:
            payload["deliveryStatus"] = delivery_status
        if delivery_status == DELIVERY_STATUS_UNKNOWN:
