...
def _private_acl_environment(path: Path) -> dict[str, str]:
    """Tạo environment tối thiểu cho Windows PowerShell ACL script.

    PowerShell 7 thêm module path riêng vào ``PSModulePath``. Nếu chuyển nguyên
    giá trị đó sang Windows PowerShell 5.1, module ``Microsoft.PowerShell.Security``
    có thể không được nạp và ``Get-Acl``/``Set-Acl`` sẽ biến mất. Bỏ biến này để
    tiến trình con tự dựng module path đúng với runtime của chính nó.
    """
    acl_env = {
        key: value
...
def set_private_file_permissions(path: Path) -> None:
    ...
    acl_env = _private_acl_environment(path)
    acl_script = r"""
