...
This pulls `httpx`, `paho-mqtt`, `attrs`, `pyotp`. The MQTT group-chat listener works out of the box. **For E2EE 1-on-1 chats** you must also build the Go bridge — see [§13](#13-building-the-e2ee-go-bridge).
...
- Network defaults: `timeout=(connect=10s, read=45s)`, **2 retries** for
  `httpx.TimeoutException` / `httpx.RequestError` (total ≤ 3 attempts).
...
Yes. The codebase provides both synchronous functions and native `async`/`await` endpoints (suffixed with `_async`, e.g., `func_async` instead of `func`) using `httpx`.
